...
// addresses. The dial used to be an unconditional insecure.NewCredentials(),
...
func TestTLSConfig_ProductionMustDecideAboutPlaintext(t *testing.T) {
	...
	cases := []struct {
		...
		{"dev, nothing set: plaintext, as today", TLSConfig{}, false},
		{"dev with TLS on", TLSConfig{Enabled: true}, false},
		{"prod with TLS on", TLSConfig{Enabled: true, IsProd: true}, false},
		{"prod, nothing set: refuses to boot", TLSConfig{IsProd: true}, true},
		{"prod, plaintext acknowledged in writing", TLSConfig{IsProd: true, AllowPlaintextInProd: true}, false},
		// The acknowledgement is not a way to skip TLS you asked for.
		{"prod with TLS on, acknowledgement irrelevant", TLSConfig{Enabled: true, IsProd: true, AllowPlaintextInProd: true}, false},
	}
...
func TestTLSConfig_ABadCAFileIsAnError(t *testing.T) {
	...
	if _, err := (TLSConfig{Enabled: true, CAFile: "/nonexistent/ca.pem"}).credentials(); err == nil {
		t.Fatal("a missing CA file was accepted")
	}

	f := t.TempDir() + "/not-a-cert.pem"
	if err := writeFile(f, "this is not a certificate"); err != nil {
		t.Fatal(err)
	}
	if _, err := (TLSConfig{Enabled: true, CAFile: f}).credentials(); err == nil {
